 ...
 * @link       https://dev.hugllc.com/index.php/Project:HUGnetLib
 */
class TableCheckPlugin extends PeriodicPluginBase
    implements PeriodicPluginInterface
{
    /** @var This is the interval at which we check in days */
    const INTERVAL=1;
    /** @var This is to register the class */
    public static $registerPlugin = array(
        "Name" => "TableCheckPlugin",
        "Type" => "periodic",
        "Class" => "TableCheckPlugin",
    );
    ...
    */
    public function __construct($config, PeriodicPlugins &$obj)
    {
        ...
        $this->generic = new GenericTable(array("group" => "default"));
        //$this->generic->verbose(10);
        // State we are here
        self::vprint(
    ...
    */
    public function main()
    {
        ...
        $this->last = $info["last"];
        $tables = $this->generic->getTables();
        foreach ($tables as $table) {
            if ($info["tables"][$table] < (time() - (self::INTERVAL * 86400))) {
                self::vprint(
                    "Checking table $table.  Last Check: "
                    .date("Y-m-d H:i:s", $info["tables"][$table]),
                    HUGnetClass::VPRINT_NORMAL
                );
                if ($this->generic->checkTable($table)) {
                    $info["tables"][$table] = time();
                }
                break;
            }
        }
